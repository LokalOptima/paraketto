...
// Unified FP8 GEMM: quantize activation, compute alpha, run CUTLASS.
...
static void fp8_gemm_impl(cudaStream_t stream,
                           ...
                           const float* w_scale,
                           half* Y,
                           uint8_t* act_buf, float* act_scale, int* amax_buf,
                           float* alpha_buf,
                           bool calibrated, bool prequantized) {
    // 1. Quantize activation FP16 → FP8 (skip if fused kernel already did it)
    if (!prequantized) {
        int elems = m * k;
        if (!calibrated)
            quantize_absmax_fp16_to_fp8(X_fp16, act_buf, act_scale, elems, amax_buf, stream);
        else
            quantize_fp8_static(X_fp16, act_buf, act_scale, elems, stream);
    }

    // 2. Compute alpha = w_scale * act_scale (only during calibration;
    //    after calibration, alpha_buf is pre-computed by fp8_compute_all_alphas)
    if (!calibrated)
        fp8_compute_alpha(alpha_buf, w_scale, act_scale, stream);

    // 3. CUTLASS FP8 GEMM: Y[m,n] = alpha * X_fp8[m,k] @ W_fp8[n,k]^T
...
int CudaModel::encode_gpu(int T_mel) {
    ...
    // FP8 GEMM helpers — CUTLASS FP8 with per-site cached activation scales
    ...
    auto gnn8 = [&](const half* X, int m, int k, const uint8_t* W, int n,
        ...
        fp8_gemm_impl(stream, X, m, k, W, n, ws, Y, fp8_act_buf,
                      &fp8_act_site_scales[si], fp8_amax_buf,
                      &fp8_alpha_products[si], cal, preq);
    };
    ...
    fp8_calibrated = true;
    // Pre-compute all alpha products now that scales are known
    fp8_compute_all_alphas(fp8_alpha_products, fp8_scales, fp8_act_site_scales, N_BLOCKS, stream);
    return T;
}
