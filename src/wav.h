...
// Shared WAV chunk parser: extracts fmt and data from a chunk stream.
// Returns false if required chunks are missing or format is unsupported.
struct WavFmt {
    uint16_t audio_format = 0, num_channels = 0, bits_per_sample = 0;
    uint32_t sample_rate = 0, data_size = 0;
    const char* data_ptr = nullptr;  // only set by memory parser
};

// Convert raw PCM samples to float based on format info.
// Returns empty WavData on unsupported format.
static inline WavData wav_decode_samples(const WavFmt& fmt, const char* raw_data) {
    WavData wav;
    wav.sample_rate = fmt.sample_rate;
    if (fmt.audio_format == 1 && fmt.bits_per_sample == 16) {
        int n = fmt.data_size / 2; wav.samples.resize(n);
        ...
        int n = fmt.data_size / 4; wav.samples.resize(n);
        memcpy(wav.samples.data(), raw_data, fmt.data_size);
    } else {
        return {};
    }
...
static inline WavData read_wav(const std::string& path) {
    ...
    if (fmt.sample_rate != 16000 && fmt.sample_rate != 24000) { fprintf(stderr, "Need 16kHz (or 24kHz), got %dHz: %s\n(resample with: ffmpeg -i input.wav -ar 16000 output.wav)\n", fmt.sample_rate, path.c_str()); std::exit(1); }

    WavData wav = wav_decode_samples(fmt, raw_data.data());
    if (wav.samples.empty()) { fprintf(stderr, "Unsupported fmt=%d bits=%d\n", fmt.audio_format, fmt.bits_per_sample); std::exit(1); }
    return wav;
}
...
static inline WavData read_wav_from_memory(const char* buf, size_t len) {
    ...
    while (pos + 8 <= len) {
        ...
        if (!memcmp(id, "fmt ", 4) && pos + 16 <= len) {
            ...
            memcpy(&fmt.sample_rate, buf + pos + 4, 4);
            memcpy(&fmt.bits_per_sample, buf + pos + 14, 2);
        } else if (!memcmp(id, "data", 4)) {
